<?php
// $Id: template-subtheme.php,v 1.1.2.1 2007/11/08 08:20:46 johnalbin Exp $
...
function _phptemplate_node($vars, $suggestions) {
  ...
  return _osulibraries_default('node', $vars, $suggestions);
}

function _phptemplate_comment($vars, $suggestions) {
  array_unshift($suggestions, 'comment'); // Not sure why I need to do this.
  return _osulibraries_default('comment', $vars, $suggestions);
}

function _phptemplate_page($vars, $suggestions) {
  return _osulibraries_default('page', $vars, $suggestions);
}

function _phptemplate_block($vars, $suggestions) {
  return _osulibraries_default('block', $vars, $suggestions);
}

function _phptemplate_box($vars, $suggestions) {
  return _osulibraries_default('box', $vars, $suggestions);
}
 ...
 */
function _osulibraries_default($hook, $variables, $suggestions = array(), $extension = '.tpl.php') {
  global $theme_engine;
  global $theme;
  global $theme_key;
