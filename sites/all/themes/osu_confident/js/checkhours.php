<?php
				...
				if(($curr_day >= strtotime($int_row['start_date']))&&($curr_day <= strtotime($int_row['end_date']))){
					switch($i){
						case 6: //sunday
							...
							$open[$i] = date('g:i a', strtotime($int_row['open_time_sun']));
							...
							$close[$i] = date('g:i a', strtotime($int_row['close_time_sun']));
							break;
						case 5: //saturday
							...
							$open[$i] = date('g:i a', strtotime($int_row['open_time_sat']));
							...
							$close[$i] = date('g:i a', strtotime($int_row['close_time_sat']));
							break;
						default:
							...
							$open[$i] = date('g:i a', strtotime($int_row['open_time_wk']));
							...
							$close[$i] = date('g:i a', strtotime($int_row['close_time_wk']));
							break;
					}
				}
			}
		...
		if(mysqli_num_rows($special_result) > 0){
			while($special_row = mysqli_fetch_assoc($special_result)){
				if(($curr_day >= strtotime($special_row['start_date']))&&($curr_day <= strtotime($special_row['end_date']))){
					...
					$open[$i] = date('g:i a', strtotime($special_row['open_time']));
					...
					$close[$i] = date('g:i a', strtotime($special_row['close_time']));
	...
	if(isset($open[$i])){
		...
		if($close[$i] == '12:15 am' || $open[$i] == '12:15 am'){
			echo '<p><strong>'.$_GET['q'].':</strong><br />'. ($open[$i] == '12:15 am' ? 'No Closing' : $open[$i]) .' - ' . ($close[$i] == '12:15 am' ? 'No Closing' : $close[$i]) . ' '.((isset($special_note[$i])) ? $special_note[$i] : '').'<br />';
		}else{
			echo '<p><strong>'.$_GET['q'].':</strong><br />'.($open[$i] == $close[$i] ? (($open[$i] == '12:00 am') ? 'Open 24 Hours': 'Closed') : $open[$i].' - '.$close[$i]).' '.((isset($special_note[$i])) ? $special_note[$i] : '').'<br />';
		}
			$j++;
			$i = ($i+1)%7;
	}
